...
async function loadSessionInfo() {
  ...
  refreshBadgeBelumLunas();
}

// Badge jumlah tiket yang belum dibayar, supaya tagihan tertunda tidak terlupakan.
async function refreshBadgeBelumLunas() {
  const el = document.getElementById('navBadgeBelumLunas');
  if (!el) return;
  try {
    const r = await fetch('/pembayaran-api/rekap-supplier');
    const d = await r.json();
    const n = d?.data?.total_tiket || 0;
    el.textContent = n;
    el.style.display = n > 0 ? '' : 'none';
  } catch (e) { }
}
window.refreshBadgeBelumLunas = refreshBadgeBelumLunas;
      ...
      <div class="header-top">
        <div class="header-brand">
          ...
          <nav class="header-nav">
            ...
            <a class="nav-item ${activePage === 'pembayaran' ? 'active' : ''}" href="/pembayaran?v=122"><svg class="icon" aria-hidden="true"><use href="#i-cash-stack"></use></svg> Pembayaran Supplier <span id="navBadgeBelumLunas" class="nav-badge" style="display:none;margin-left:auto;min-width:20px;border-radius:9999px;background:#ef4444;color:#fff;padding:2px 6px;text-align:center;font-size:10px;font-weight:700;line-height:1"></span></a>
