  ...
  try {
    ...
    const formattedRows = rows.map(row => {
      ...
      return {
        ...
        // Hanya entri yang benar-benar diinput tangan di halaman Keuangan yang boleh
        // dihapus. Entri dari timbangan, pembayaran supplier, dan mutasi hutang
        // dikunci karena punya pasangan catatan di tempat lain.
        is_manual: !row.id_transaksi && !row.id_pembayaran && !row.id_hutang_ledger,
  ...
  try {
    ...
    if (entry.id_pembayaran) {
      return jsonResponse(res, false,
        'Tidak bisa menghapus entry pembayaran supplier dari sini. Batalkan pembayarannya di menu Pembayaran Supplier agar tiketnya ikut dikembalikan.');
    }
