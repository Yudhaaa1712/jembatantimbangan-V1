...
// Migration: Pastikan pengaturan_gaji punya kolom tarif & baris default (tarif_pemuat = 20).
// Upah TKBM = netto RAM (kg) × tarif_pemuat ÷ jumlah pekerja. Default pemuat Rp 20/kg, tetap bisa diubah.
try {
  ...
  if (cnt === 0) {
    db.prepare("INSERT INTO pengaturan_gaji (tarif_per_kg, tarif_supir, tarif_pemuat) VALUES (0, 0, 20)").run();
    console.log('[DB Migration] Seed pengaturan_gaji default (tarif_pemuat = 20)');
  }
} catch (e) {
  console.error('[DB Migration] pengaturan_gaji default error:', e.message);
}

// Default fitur aktif. Dipakai untuk seed instalasi baru DAN untuk melengkapi
// kunci yang belum ada pada instalasi lama (mis. 'upah' yang ditambahkan
// belakangan) supaya menunya langsung muncul tanpa harus di-toggle dulu.
...
// Pastikan baris active_features selalu ada dan lengkap kuncinya
try {
  const row = db.prepare("SELECT setting_value FROM settings WHERE setting_key = 'active_features'").get();
  let features = { ...DEFAULT_ACTIVE_FEATURES };
  let needWrite = true;

  if (row && row.setting_value) {
    let saved = {};
    try { saved = JSON.parse(row.setting_value) || {}; } catch (e) { saved = {}; }
    const missing = Object.keys(DEFAULT_ACTIVE_FEATURES).filter(k => saved[k] === undefined);
    features = { ...DEFAULT_ACTIVE_FEATURES, ...saved };
    needWrite = missing.length > 0;
    if (needWrite) {
      console.log('[DB Migration] Melengkapi active_features yang belum ada: ' + missing.join(', '));
    }
  } else {
    console.log('[DB Migration] Seed active_features default');
  }

  if (needWrite) {
    db.prepare(
      `INSERT INTO settings (setting_key, setting_value) VALUES ('active_features', ?)
       ON CONFLICT(setting_key) DO UPDATE SET setting_value = excluded.setting_value`
    ).run(JSON.stringify(features));
  }
} catch (e) {
  console.error('[DB Migration] active_features error:', e.message);
}

// Migration satu kali: tarif pemuat TKBM Rp 30/kg → Rp 20/kg.
// Hanya dijalankan sekali (ditandai lewat tabel settings). Kalau operator
// mengubah tarif secara sengaja setelah ini, nilainya TIDAK akan ditimpa lagi.
try {
  const FLAG = 'migrasi_tarif_pemuat_20';
  const sudah = db.prepare("SELECT setting_value FROM settings WHERE setting_key = ?").get(FLAG);
  if (!sudah) {
    const info = db.prepare("UPDATE pengaturan_gaji SET tarif_pemuat = 20 WHERE tarif_pemuat = 30").run();
    db.prepare(
      `INSERT INTO settings (setting_key, setting_value) VALUES (?, ?)
       ON CONFLICT(setting_key) DO UPDATE SET setting_value = excluded.setting_value`
    ).run(FLAG, new Date().toISOString());
    if (info.changes > 0) {
      console.log('[DB Migration] Tarif pemuat TKBM diubah 30 → 20 (' + info.changes + ' baris)');
    }
  }
} catch (e) {
  console.error('[DB Migration] tarif_pemuat 30→20 error:', e.message);
}
