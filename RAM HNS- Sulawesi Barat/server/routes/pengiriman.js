...
router.post('/timbang1', async (req, res) => {
  try {
    ...
    if (berat1 <= 0) {
      return jsonResponse(res, false, 'Berat Timbangan Masuk harus lebih dari 0');
    }
...
router.post('/timbang2', async (req, res) => {
  try {
    ...
    // Alur pengiriman: Timbang 1 = mobil kosong (TARA), Timbang 2 = mobil isi (BRUTO).
    // Netto = Bruto - Tara, jadi berat Timbang 2 wajib lebih besar dari Timbang 1.
    ...
    if (netto_ram <= 0) {
      return jsonResponse(
        res,
        false,
        `Berat Timbang 2 (mobil isi) harus lebih besar dari Timbang 1 (mobil kosong). ` +
        `Timbang 1 = ${berat1.toLocaleString('id-ID')} kg, Timbang 2 = ${berat2.toLocaleString('id-ID')} kg. ` +
        `Netto tidak boleh nol atau minus.`
      );
    }
...
router.get('/export-excel', async (req, res) => {
  try {
    ...
    const companyName = settingRow ? settingRow.setting_value : 'Laporan Timbangan Pengiriman';
    ...
    let html = `
      ...
      <body>
        <h3>${companyName} - Data Timbangan Pengiriman</h3>
        <table border="1">
          <thead>
