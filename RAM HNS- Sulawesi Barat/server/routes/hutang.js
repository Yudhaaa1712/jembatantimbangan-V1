...
/**
 * Mutasi hutang manual sekaligus mencatatnya di buku kas.
 *
 *   tambah → perusahaan menyerahkan uang pinjaman   → KAS KELUAR
 *   bayar  → pihak terkait mengembalikan pinjaman   → KAS MASUK
 *
 * Hanya mutasi manual yang menyentuh kas. Potongan otomatis dari timbangan
 * (sumber 'timbangan') dan potongan gaji (sumber 'gaji') sudah dibukukan oleh
 * modulnya masing-masing, jadi tidak dicatat lagi di sini agar tidak dobel.
 */
async function mutasiHutang(req, res, jenis, defaultKet) {
  try {
    ...
    const party = await queryOne(
      `SELECT ${c.nameCol} AS nama FROM ${c.table} WHERE id = ?${f.sql}`,
      [id, ...f.params]
    );
    const namaPihak = (party?.nama || '').toUpperCase() || '-';
    ...
    try {
      ...
      // Catat arus kasnya
      const kasJenis = jenis === 'bayar' ? 'masuk' : 'keluar';
      const kasLabel = jenis === 'bayar'
        ? `Penerimaan Pembayaran Hutang ${c.label} - ${namaPihak}`
        : `Pemberian Pinjaman ${c.label} - ${namaPihak}`;
      const catatanTambahan = cleanInput(req.body.keterangan || '') ? ` (${keterangan})` : '';
      catatKasTx(tx, {
        jenis: kasJenis,
        jumlah,
        keterangan: kasLabel + catatanTambahan,
  ...
  try {
    ...
    try {
      ...
      if (initialDebt > 0) {
        ...
        // Uang pinjaman diserahkan saat pendaftaran → kas keluar
        catatKasTx(tx, {
          jenis: 'keluar',
          jumlah: initialDebt,
          keterangan: `Pemberian Pinjaman ${c.label} - ${nama} (hutang awal saat pendaftaran)`,
          operatorId: req.session.user_id
        });
      }
      tx.commit();
