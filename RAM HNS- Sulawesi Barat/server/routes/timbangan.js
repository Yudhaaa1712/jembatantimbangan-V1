...
router.get('/data', async (req, res) => {
  try {
    ...
    const defaultHargaRow = await queryOne(`SELECT setting_value FROM settings WHERE setting_key = 'default_harga_umum'`);
    const defaultHargaUmum = parseFloat(defaultHargaRow?.setting_value) || 0;
    ...
    res.json({
      ...
      data: {
        ...
        default_harga_umum: defaultHargaUmum,
      ...
      case 'save_timbangan2': {
        ...
        try {
          ...
          if (totalAkhir > 0) {
            ...
            // Keterangan kas: "PEMBELIAN TBS - UJANG" / "PENJUALAN BRONDOLAN - SIRUN"
            const materialLabel = (tiketInfo?.jenis_material || 'Buah').toUpperCase();
            const pihakLabel = (tiketInfo?.nama_supplier || tiketInfo?.nama_supir || '').toUpperCase();
            const kasKeterangan = `${isBeli ? 'PEMBELIAN' : 'PENJUALAN'} ${materialLabel}`
              + (pihakLabel ? ` - ${pihakLabel}` : '')
              + (tiketInfo?.no_tiket ? ` (${tiketInfo.no_tiket})` : '');
