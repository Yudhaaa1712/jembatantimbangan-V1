...
const { formatPeriode, kasKeterangan } = require('../helpers/format');
const { catatKasTx } = require('../helpers/kasHelper');
...
router.post('/toggle-status-bayar', async (req, res) => {
  try {
    ...
    const trip = await queryOne(
      `SELECT no_surat_jalan, nama_supir, netto_pabrik, netto_ram,
              COALESCE(harga_per_kg, 0)   AS harga_per_kg,
              COALESCE(pinjaman, 0)       AS pinjaman,
              COALESCE(biaya_es_jalan, 0) AS biaya_es_jalan,
              COALESCE(status_bayar, 'belum_bayar') AS status_bayar
       FROM pengiriman_pabrik WHERE id = ?`,
      [id]
    );
    if (!trip) return jsonResponse(res, false, 'Data pengiriman tidak ditemukan');

    // Sudah pada status yang diminta → tidak perlu mencatat kas lagi (hindari dobel)
    if (trip.status_bayar === statusBayar) {
      return jsonResponse(res, true, statusBayar === 'lunas' ? 'Ditandai Lunas' : 'Ditandai Belum Bayar', { id, status_bayar: statusBayar });
    }

    // Hasil bersih = tonase angkut × harga/kg − pinjaman − biaya es+jalan
    const tonase = parseFloat(trip.netto_pabrik ?? trip.netto_ram ?? 0) || 0;
    const hasilBersih = Math.max(
      0,
      (tonase * parseFloat(trip.harga_per_kg)) - parseFloat(trip.pinjaman) - parseFloat(trip.biaya_es_jalan)
    );
    ...
    try {
      ...
      // Tandai Lunas → uang keluar ke supir. Dibatalkan → uang dikembalikan ke kas.
      if (hasilBersih > 0) {
        const konteks = `${trip.no_surat_jalan || 'SJ -'}`;
        catatKasTx(tx, {
          jenis: statusBayar === 'lunas' ? 'keluar' : 'masuk',
          jumlah: hasilBersih,
          keterangan: statusBayar === 'lunas'
            ? kasKeterangan('Pembayaran Ongkos Angkut Supir', trip.nama_supir, konteks)
            : kasKeterangan('Pembatalan Pembayaran Ongkos Angkut Supir', trip.nama_supir, konteks),
          operatorId: req.session.user_id
        });
      }
    ...
    try {
      ...
      if (finalCleanWage > 0) {
        ...
        tx.execute(
          ...
          [finalCleanWage, kasKeterangan('Pembayaran Upah Supir', supir.nama_supir, formatPeriode(start_date, end_date)), saldoSesudah, req.session.user_id]
        );
      }
    ...
    try {
      ...
      if (gaji.gaji_bersih > 0) {
        ...
        tx.execute(
          ...
          [gaji.gaji_bersih, kasKeterangan('Pembatalan Pembayaran Upah Supir', supir.nama_supir, `Ref #${id}, ${formatPeriode(gaji.periode_mulai, gaji.periode_akhir)}`), saldoSesudah, req.session.user_id]
        );
      }
...
router.get('/tkbm-gaji-preview', async (req, res) => {
  try {
    ...
    const sql = `
      SELECT pt.id as id_pengiriman_tkbm, p.id as id_pengiriman, p.no_surat_jalan as no_tiket, p.tanggal, p.no_polisi, COALESCE(p.netto_ram,0) AS netto_ram, p.nama_pabrik,
...
router.post('/tkbm-gaji', requireRole('admin'), async (req, res) => {
  try {
    ...
    try {
      // Validasi ulang
      const sql = `
        SELECT pt.id, COALESCE(p.netto_ram,0) AS netto_ram, (SELECT COUNT(*) FROM pengiriman_tkbm WHERE id_pengiriman = p.id) as jumlah_pekerja
        FROM pengiriman_tkbm pt
        JOIN pengiriman_pabrik p ON pt.id_pengiriman = p.id
        WHERE pt.id_tkbm = ? AND p.status = 'selesai' AND pt.id_gaji_tkbm IS NULL AND p.tanggal BETWEEN ? AND ?
      `;
      ...
      tx.execute(
        `INSERT INTO kas (tanggal, jenis, jumlah, keterangan, saldo_setelah, operator_id)
         VALUES (date('now', 'localtime'), 'keluar', ?, ?, ?, ?)`,
        [(calculatedTotalGaji - potLainnya), kasKeterangan('Pembayaran Upah TKBM', tkbm.nama_karyawan, formatPeriode(start_date, end_date)), saldoSesudah, req.session.user_id]
      );
...
router.get('/tkbm-gaji/:id', async (req, res) => {
  try {
    ...
    const sql = `
      SELECT pt.id, p.no_surat_jalan as no_tiket, p.tanggal, p.no_polisi, COALESCE(p.netto_ram,0) AS netto_ram, p.nama_pabrik,
             (SELECT COUNT(*) FROM pengiriman_tkbm WHERE id_pengiriman = p.id) as jumlah_pekerja
      FROM pengiriman_tkbm pt
      JOIN pengiriman_pabrik p ON pt.id_pengiriman = p.id
...
router.delete('/tkbm-gaji/:id', requireRole('admin'), async (req, res) => {
  try {
    ...
    const tkbmInfo = await queryOne(`SELECT nama_karyawan FROM karyawan_tkbm WHERE id = ?`, [gaji.id_tkbm]);
    ...
    try {
      ...
      if (nominal > 0) {
        ...
        tx.execute(
          `INSERT INTO kas (tanggal, jenis, jumlah, keterangan, saldo_setelah, operator_id)
           VALUES (date('now', 'localtime'), 'masuk', ?, ?, ?, ?)`,
          [nominal, kasKeterangan('Pembatalan Pembayaran Upah TKBM', tkbmInfo?.nama_karyawan, `Ref #${id}, ${formatPeriode(gaji.periode_mulai, gaji.periode_akhir)}`), saldoSesudah, req.session.user_id]
        );
      }
...
async function getTkbmTripsRoster(dateSql, dateParams) {
  const trips = await query(`
    SELECT pp.id, pp.tanggal, COALESCE(pp.netto_ram,0) AS netto_ram, pp.no_polisi, pp.nama_pabrik,
           GROUP_CONCAT(pt.id_tkbm) AS worker_ids
    FROM pengiriman_pabrik pp
    JOIN pengiriman_tkbm pt ON pt.id_pengiriman = pp.id
    WHERE pp.status IN ('timbang_2','selesai') AND COALESCE(pp.netto_ram,0) > 0 AND ${dateSql}
    GROUP BY pp.id
...
async function getTarifPemuat() {
  const config = await queryOne(`SELECT tarif_pemuat FROM pengaturan_gaji LIMIT 1`);
  return config && config.tarif_pemuat != null ? parseFloat(config.tarif_pemuat) : 20;
}
  ...
  try {
    ...
    try {
      ...
      tx.execute(
        `INSERT INTO kas (tanggal, jenis, jumlah, keterangan, saldo_setelah, operator_id)
         VALUES (date('now', 'localtime'), 'keluar', ?, ?, ?, ?)`,
        [total, kasKeterangan('Potongan Pinjaman dari Upah Supir', supir.nama, formatPeriode(startDate, endDate)), saldoSebelum - total, req.session.user_id]
      );
  ...
  try {
    ...
    try {
      ...
      tx.execute(
        `INSERT INTO kas (tanggal, jenis, jumlah, keterangan, saldo_setelah, operator_id)
         VALUES (date('now', 'localtime'), 'masuk', ?, ?, ?, ?)`,
        [entry.jumlah, kasKeterangan(`Pembatalan Potongan Pinjaman dari Upah ${cfg.label}`, party ? party.nama : '', `Ref #${ledgerId}`), saldoSesudah, req.session.user_id]
      );
