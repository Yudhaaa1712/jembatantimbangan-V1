...
// GET supir-aktif
router.get(['/supir-aktif', '/supir/aktif'], async (req, res) => {
  try {
...
// GET supplier-aktif
router.get(['/supplier-aktif', '/supplier/aktif'], async (req, res) => {
  try {
    const data = await query(
...
// POST supplier/add
router.post(['/supplier/add', '/supplier-add'], async (req, res) => {
  try {
    const name = cleanInput(req.body.nama_supplier || req.body.nama_supir || '').trim().toUpperCase();
    const telepon = cleanInput(req.body.no_telepon || '').trim();
    const alamat = cleanInput(req.body.alamat || '').trim();
    const initialDebt = parseFloat(req.body.initial_debt) || 0;
    ...
    let supplier = await queryOne(`SELECT * FROM supplier WHERE UPPER(nama_supplier) = ?`, [name]);
    let supplierId;
    ...
    try {
      if (supplier) {
        ...
        if (initialDebt > 0) {
          const newHutang = (supplier.total_hutang || 0) + initialDebt;
          tx.execute(`UPDATE supplier SET total_hutang = ?, hutang_terakhir_update = datetime('now', 'localtime') WHERE id = ?`, [newHutang, supplierId]);
          tx.execute(
            `INSERT INTO hutang_supplier_history (id_supplier, tanggal, jenis, jumlah, keterangan, saldo_setelah, operator_id) 
             VALUES (?, date('now', 'localtime'), 'tambah', ?, 'Hutang awal saat pendaftaran', ?, ?)`,
            [supplierId, initialDebt, newHutang, req.session.user_id]
          );
        }
      } else {
        ...
        const [result] = tx.execute(
          `INSERT INTO supplier (kode_supplier, nama_supplier, no_telepon, alamat, total_hutang, status, is_temporary) VALUES (?, ?, ?, ?, ?, 'active', 0)`,
          [kode, name, telepon, alamat, initialDebt]
        );
        ...
        tx.execute(
          ...
          [supplierId, initialDebt, initialDebt > 0 ? 'Hutang awal saat pendaftaran' : 'Pendaftaran Supplier Baru', initialDebt, req.session.user_id]
        );
      }
...
// ─── GENERAL BAYAR & TAMBAH (Dispatched by route context) ───────────────────

// POST bayar
router.post(['/bayar', '/supplier/bayar'], async (req, res) => {
  try {
    const isSupplier = req.baseUrl.includes('supplier') || !!req.body.id_supplier;
    const amount = parseFloat(req.body.jumlah);
    const tgl = req.body.tanggal || new Date().toISOString().split('T')[0];
    const ket = req.body.keterangan || 'Pembayaran Manual';

    if (isSupplier) {
      const id_supplier = parseInt(req.body.id_supplier || req.body.id_supir);
      if (!id_supplier || isNaN(amount) || amount <= 0) return jsonResponse(res, false, 'Data tidak valid');
      ...
      const newHutang = Math.max(0, (supplier.total_hutang || 0) - amount);
      const tx = beginTransaction();
      try {
        ...
        tx.execute(
          `INSERT INTO hutang_supplier_history (id_supplier, tanggal, jenis, jumlah, keterangan, saldo_setelah, operator_id) 
           VALUES (?, ?, 'bayar', ?, ?, ?, ?)`,
          [id_supplier, tgl, amount, ket, newHutang, req.session.user_id]
        );
        tx.commit();
      ...
      const id_supir = parseInt(req.body.id_supir);
      if (!id_supir || isNaN(amount) || amount <= 0) return jsonResponse(res, false, 'Data tidak valid');
      ...
      const newHutang = Math.max(0, (supir.total_hutang || 0) - amount);
      const tx = beginTransaction();
      try {
        ...
        tx.execute(
          `INSERT INTO hutang_supir_history (id_supir, tanggal, jenis, jumlah, keterangan, saldo_setelah, operator_id) 
           VALUES (?, ?, 'bayar', ?, ?, ?, ?)`,
          [id_supir, tgl, amount, ket, newHutang, req.session.user_id]
        );
        tx.commit();
        return jsonResponse(res, true, 'Pembayaran hutang supir berhasil dicatat');
      } catch (txErr) {
        tx.rollback();
        throw txErr;
      }
...
// POST tambah
router.post(['/tambah', '/supplier/tambah'], async (req, res) => {
  try {
    const isSupplier = req.baseUrl.includes('supplier') || !!req.body.id_supplier;
    const amount = parseFloat(req.body.jumlah);
    const tgl = req.body.tanggal || new Date().toISOString().split('T')[0];
    const ket = req.body.keterangan || 'Penambahan Manual';
    ...
    if (isSupplier) {
      ...
      const newHutang = (supplier.total_hutang || 0) + amount;
      const tx = beginTransaction();
      try {
        tx.execute(`UPDATE supplier SET total_hutang = ?, hutang_terakhir_update = datetime('now', 'localtime') WHERE id = ?`, [newHutang, id_supplier]);
        tx.execute(
      ...
      const newHutang = (supir.total_hutang || 0) + amount;
      const tx = beginTransaction();
      try {
        tx.execute(`UPDATE supir SET total_hutang = ?, updated_at = datetime('now', 'localtime') WHERE id = ?`, [newHutang, id_supir]);
        tx.execute(
        ...
        tx.commit();
        return jsonResponse(res, true, 'Penambahan hutang supir berhasil dicatat');
      } catch (txErr) {
        tx.rollback();
        throw txErr;
      }
...
// GET export-excel
router.get(['/export-excel', '/supplier/export-excel'], async (req, res) => {
  try {
    const isSupplier = req.baseUrl.includes('supplier');
    if (isSupplier) {
      ...
      supplierList.forEach((s, idx) => {
        ...
        html += `
          <tr>
              ...
              <td class="text-center">${(s.status || 'active').toUpperCase()}</td>
          </tr>`;
      });
...
<head>
    <meta charset="utf-8">
    <title>Laporan Rekapan Hutang Supir</title>
    <style>
        body { font-family: Arial, sans-serif; }
        .header { text-align: center; margin-bottom: 20px; }
        .title { font-size: 18px; font-weight: bold; text-decoration: underline; }
...
<body>
<div class="header">
    <div class="title">LAPORAN REKAPAN HUTANG SUPIR</div>
    <div>Tanggal Cetak: ${new Date().toLocaleDateString('id-ID')}</div>
</div>
<table class="data-table">
    <thead>
