...
const { buildKeterangan, periodeRef, recalculateKasBalances } = require('../helpers/kasHelper');
...
// ─── Tarif pemuat (TKBM) ─────────────────────────────────────────────────────
// Dipakai tab "Rekap Gaji TKBM" supaya angka pengali tonase bisa diubah dari
// halamannya sendiri tanpa harus buka tab Pengaturan — dan tanpa menimpa
// tarif petani/supir yang tersimpan di baris yang sama.
router.get('/tarif-pemuat', async (req, res) => {
  try {
    return jsonResponse(res, true, 'Tarif pemuat', { tarif_pemuat: await getTarifPemuat() });
  } catch (err) {
    return jsonResponse(res, false, err.message);
  }
});

router.post('/tarif-pemuat', requireRole('admin'), async (req, res) => {
  try {
    const tarif = parseFloat(String(req.body.tarif_pemuat).replace(/[^0-9.]/g, ''));
    if (isNaN(tarif) || tarif < 0) return jsonResponse(res, false, 'Tarif tidak valid');

    const existing = await queryOne(`SELECT id FROM pengaturan_gaji LIMIT 1`);
    if (existing) {
      await query(
        `UPDATE pengaturan_gaji SET tarif_pemuat = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
        [tarif, existing.id]
      );
    } else {
      await query(`INSERT INTO pengaturan_gaji (tarif_per_kg, tarif_supir, tarif_pemuat) VALUES (0, 0, ?)`, [tarif]);
    }
    return jsonResponse(res, true, `Tarif upah TKBM disimpan: Rp ${tarif.toLocaleString('id-ID')} / kg`,
      { tarif_pemuat: tarif });
  } catch (err) {
    return jsonResponse(res, false, err.message);
  }
  ...
  try {
    ...
    try {
      ...
      if (finalCleanWage > 0) {
        ...
        tx.execute(
          `INSERT INTO kas (tanggal, jenis, jumlah, keterangan, id_gaji_supir, saldo_setelah, operator_id)
           VALUES (date('now', 'localtime'), 'keluar', ?, ?, ?, ?, ?)`,
          [finalCleanWage,
           buildKeterangan('UPAH SUPIR', supir.nama_supir, periodeRef(start_date, end_date)),
           gajiId, saldoSesudah, req.session.user_id]
        );
      }
    ...
    try {
      ...
      if (gaji.gaji_bersih > 0) {
        ...
        tx.execute(
          ...
          [gaji.gaji_bersih,
           buildKeterangan('BATAL UPAH SUPIR', supir.nama_supir, `GJS#${id}`),
           saldoSesudah, req.session.user_id]
        );
      }
...
router.post('/tkbm-gaji', requireRole('admin'), async (req, res) => {
  try {
    ...
    try {
      ...
      tx.execute(
        `INSERT INTO kas (tanggal, jenis, jumlah, keterangan, id_gaji_tkbm, saldo_setelah, operator_id)
         VALUES (date('now', 'localtime'), 'keluar', ?, ?, ?, ?, ?)`,
        [(calculatedTotalGaji - potLainnya),
         buildKeterangan('UPAH TKBM', tkbm.nama_karyawan, periodeRef(start_date, end_date)),
         gajiId, saldoSesudah, req.session.user_id]
      );
...
router.delete('/tkbm-gaji/:id', requireRole('admin'), async (req, res) => {
  try {
    ...
    if (!gaji) return jsonResponse(res, false, 'Data gaji tidak ditemukan');
    const tkbmBatal = await queryOne(`SELECT nama_karyawan FROM karyawan_tkbm WHERE id = ?`, [gaji.id_tkbm]);
      ...
      if (nominal > 0) {
        ...
        tx.execute(
          `INSERT INTO kas (tanggal, jenis, jumlah, keterangan, saldo_setelah, operator_id)
           VALUES (date('now', 'localtime'), 'masuk', ?, ?, ?, ?)`,
          [nominal,
           buildKeterangan('BATAL UPAH TKBM', tkbmBatal ? tkbmBatal.nama_karyawan : '', `GJT#${id}`),
           saldoSesudah, req.session.user_id]
        );
      }
...
// Tarif pengali tonase untuk upah TKBM (Rp/kg). Selalu dari database — tidak ada
// angka bawaan di kode, supaya tiap instalasi mengisi tarifnya sendiri.
async function getTarifPemuat() {
  const config = await queryOne(`SELECT tarif_pemuat FROM pengaturan_gaji LIMIT 1`);
  return config && config.tarif_pemuat != null ? parseFloat(config.tarif_pemuat) : 0;
}
...
router.post('/proses-potong-supir', requireRole('admin'), async (req, res) => {
  try {
    ...
    // Ongkos angkut per trip ikut diambil karena uang yang diserahkan ke supir
    // juga harus tercatat di Buku Kas (lihat pencatatan bruto di bawah).
    const trips = await query(
      `SELECT pp.id, COALESCE(pp.pinjaman,0) AS pinjaman,
              COALESCE(pp.netto_pabrik,0) * COALESCE(pp.harga_per_kg,0) AS ongkos,
              COALESCE(pp.biaya_es_jalan,0) AS es_jalan
       FROM pengiriman_pabrik pp
       WHERE ${dateSql}
    ...
    try {
      ...
      // 2. Catat ke Buku Kas secara BRUTO.
      //
      // Sebelumnya baris ini ditulis sebagai uang KELUAR sebesar potongan —
      // terbalik, karena potong pinjaman itu hutang yang DIBAYAR supir, bukan
      // uang yang kita keluarkan. Sekaligus ongkos angkutnya dulu tidak pernah
      // masuk Buku Kas sama sekali, padahal uangnya benar-benar diserahkan.
      //
      // Sekarang keduanya dicatat berpasangan:
      //     KELUAR  ongkos angkut trip (setelah biaya es & jalan)
      //     MASUK   potongan pinjaman
      // Selisihnya = uang yang benar-benar diterima supir.
      const ongkosBruto = trips.reduce(
        (s, t) => s + Math.max(0, (parseFloat(t.ongkos) || 0) - (parseFloat(t.es_jalan) || 0)), 0);

      const [lastKas] = tx.execute(`SELECT saldo_setelah FROM kas ORDER BY id DESC LIMIT 1`);
      let saldoBerjalan = lastKas.length > 0 ? parseFloat(lastKas[0].saldo_setelah) : 0;
      const refPeriode = periodeRef(startDate, endDate);

      const barisKas = [];
      if (ongkosBruto > 0) {
        barisKas.push({
          jenis: 'keluar', jumlah: ongkosBruto,
          keterangan: buildKeterangan('ONGKOS ANGKUT SUPIR', supir.nama, refPeriode)
        });
      }
      barisKas.push({
        jenis: 'masuk', jumlah: total,
        keterangan: buildKeterangan('POTONG PINJAMAN SUPIR', supir.nama, refPeriode)
      });

      for (const b of barisKas) {
        saldoBerjalan += (b.jenis === 'masuk' ? 1 : -1) * b.jumlah;
        tx.execute(
          `INSERT INTO kas (tanggal, jenis, jumlah, keterangan, id_hutang_ledger, saldo_setelah, operator_id)
           VALUES (date('now', 'localtime'), ?, ?, ?, ?, ?, ?)`,
          [b.jenis, b.jumlah, b.keterangan, ledgerId, saldoBerjalan, req.session.user_id]
        );
      }
...
router.post('/potong-pinjaman/batal', requireRole('admin'), async (req, res) => {
  try {
    ...
    try {
      ...
      // 4. Hapus SELURUH baris kas milik potongan ini (ongkos angkut keluar +
      //    potongan pinjaman masuk). Keduanya ditandai id_hutang_ledger yang
      //    sama saat dibuat, jadi pembatalannya bersih dan tidak menyisakan
      //    baris pasangan yang membuat saldo melenceng.
      const [barisKas] = tx.execute(
        `SELECT id FROM kas WHERE id_hutang_ledger = ?`, [ledgerId]);
      tx.execute(`DELETE FROM kas WHERE id_hutang_ledger = ?`, [ledgerId]);

      tx.commit();
      // Saldo berjalan seluruh baris dihitung ulang setelah penghapusan
      await recalculateKasBalances();
      return jsonResponse(res, true,
        `Potong pinjaman berhasil dibatalkan. ${barisKas.length} baris di Laporan Keuangan ikut ditarik kembali.`);
    } catch (txErr) {
      tx.rollback();
      throw txErr;
    }
