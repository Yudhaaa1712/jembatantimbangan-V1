...
router.get('/receipt/:no_tiket', async (req, res) => {
  try {
    ...
    const settings = await query(`SELECT setting_key, setting_value FROM settings WHERE setting_key IN ('company_name','company_address','company_phone','company_logo')`);
    const settingsMap = {};
    ...
    if (data.is_langsir === 1) {
      // Struk langsir perlu menampilkan tara & netto tiap trip, bukan bruto saja,
      // supaya supplier bisa memeriksa sendiri bahwa tara dipotong per trip.
      langsirTrips = await query(
        `SELECT urutan, no_polisi, nama_supir, berat_bruto, berat_tara, berat_netto, waktu_timbang, waktu_tara
         FROM transaksi_timbangan_langsir WHERE id_transaksi = ? ORDER BY urutan ASC, id ASC`, [data.id]);
    }
...
router.post('/cancel', requireRole('admin', 'operator'), async (req, res) => {
  try {
    ...
    const trx = await queryOne(`SELECT id, no_tiket, status, total_harga, potongan_hutang_rp, id_pembayaran FROM transaksi_timbangan WHERE id = ? LIMIT 1`, [id]);
    if (!trx) return jsonResponse(res, false, 'Data tidak ditemukan');

    // Tiket yang sudah masuk batch pembayaran tidak boleh diutak-atik langsung:
    // baris kasnya milik pembayaran (bukan tiket), jadi refundKas di bawah tidak
    // akan menemukannya dan saldo kas jadi melenceng. Batalkan pembayarannya dulu.
    if (trx.id_pembayaran) {
      const bayar = await queryOne(
        `SELECT nama_supplier, tanggal, status FROM pembayaran WHERE id = ? LIMIT 1`, [trx.id_pembayaran]
      );
      if (bayar && bayar.status === 'aktif') {
        return jsonResponse(res, false,
          `Tiket ini sudah dilunasi dalam pembayaran ke ${bayar.nama_supplier || 'supplier'} tanggal ${bayar.tanggal}. Hapus pembayaran tersebut dulu di menu Pembayaran.`);
      }
    }

    // ── Kembalikan kas jika transaksi sudah pernah menggerakkan kas ──
    // Baris pembalik selalu berlawanan arah dengan baris aslinya:
    //   pembelian (keluar) dibatalkan → uang kembali masuk
    //   penjualan (masuk)  dibatalkan → uang keluar lagi
    // Sebelumnya hanya baris 'keluar' yang dicari, sehingga pembatalan tiket
    // PENJUALAN meninggalkan uang masuk di kas walaupun tiketnya sudah batal.
    async function refundKas(idTransaksi, noTiket) {
      try {
        const kasRecords = await query(
          `SELECT id, jenis, jumlah FROM kas WHERE id_transaksi = ? ORDER BY id ASC`,
          [idTransaksi]
        );
        if (!kasRecords.length) return;

        // Saldo bersih yang masih tertinggal dari tiket ini (masuk - keluar).
        // Bila 0, tiket sudah pernah dibalik sebelumnya → tidak perlu diulang.
        const net = kasRecords.reduce(
          (s, k) => s + (k.jenis === 'masuk' ? 1 : -1) * (parseFloat(k.jumlah) || 0), 0);
        if (Math.abs(net) < 0.5) return;

        const info = await queryOne(
          `SELECT nama_supir, jenis_material FROM transaksi_timbangan WHERE id = ? LIMIT 1`, [idTransaksi]);

        // Saldo terakhir absolut, bukan "saldo terakhir hari ini" — kalau hari ini
        // belum ada baris kas sama sekali, cara lama memulai saldo dari 0 dan
        // membuat kolom saldo di Buku Kas melompat.
        ...
        const jenisBalik = net > 0 ? 'keluar' : 'masuk';
        const jumlahRefund = Math.abs(net);
        const saldoSesudah = jenisBalik === 'masuk' ? saldoSebelum + jumlahRefund : saldoSebelum - jumlahRefund;

        await query(
          `INSERT INTO kas (tanggal, jenis, jumlah, keterangan, id_transaksi, no_tiket, saldo_setelah, operator_id)
           VALUES (CURDATE(), ?, ?, ?, ?, ?, ?, ?)`,
          [jenisBalik, jumlahRefund,
           buildKeterangan('BATAL TRANSAKSI', info?.nama_supir, noTiket),
           idTransaksi, noTiket, saldoSesudah, user.id]
        );
        // Baris pembalik dicatat hari ini walau tiketnya lama → saldo berjalan
        // baris-baris sesudahnya harus dihitung ulang.
        await recalculateKasBalances();
        console.log(`[Kas] Balik ${jenisBalik}: Rp ${jumlahRefund.toLocaleString('id-ID')} (${noTiket}).`);
      } catch (kasErr) {
    ...
    /**
     * Kembalikan potongan hutang otomatis milik sebuah tiket ke buku besar.
     *
     * Dulu ini hanya dijalankan pada actionType 'delete', sehingga tiket yang
     * DIBATALKAN atau DIULANG TIMBANG membuat hutang supir/supplier tetap
     * berkurang padahal tiketnya sudah tidak berlaku — saldo di Manajemen Hutang
     * jadi tidak sinkron dengan transaksinya.
     *
     * Baris pembalik ditandai jenis 'tambah' + sumber 'timbangan', dan hanya
     * dibuat bila potongan aslinya belum pernah dibalik (selisih bayar - tambah).
     */
    async function kembalikanPotonganHutang(idTransaksi, noTiket) {
      const ledger = await query(
        `SELECT party_type, party_id, jenis, jumlah FROM hutang_ledger
         WHERE id_referensi = ? AND sumber = 'timbangan'`, [idTransaksi]
      );
      if (!ledger.length) return [];

      // Sisa potongan per pihak = total 'bayar' - total 'tambah' (pembalik)
      const sisa = new Map();
      for (const l of ledger) {
        const key = `${l.party_type}|${l.party_id}`;
        const arah = l.jenis === 'bayar' ? 1 : -1;
        sisa.set(key, (sisa.get(key) || 0) + arah * (parseFloat(l.jumlah) || 0));
      }
      return [...sisa.entries()]
        .filter(([, v]) => v > 0.5)
        .map(([key, v]) => {
          const [party_type, party_id] = key.split('|');
          return { party_type, party_id: parseInt(party_id), jumlah: v, no_tiket: noTiket };
        });
    }

    /** Tulis baris pembalik hutang di dalam satu transaksi DB. */
    function tulisPembalikHutang(tx, daftar) {
      for (const p of daftar) {
        catatHutangTx(tx, {
          type: p.party_type, partyId: p.party_id, jenis: 'tambah', jumlah: p.jumlah,
          keterangan: `Pembatalan tiket ${p.no_tiket || '#' + id}`,
          idReferensi: id, sumber: 'timbangan', operatorId: req.session.user_id
        });
      }
    }

    /** Versi mandiri (punya transaksi DB sendiri) untuk jalur batal & ulang timbang. */
    async function balikkanHutangTiket(idTransaksi, noTiket) {
      try {
        const daftar = await kembalikanPotonganHutang(idTransaksi, noTiket);
        if (!daftar.length) return;
        const tx = beginTransaction();
        try {
          tulisPembalikHutang(tx, daftar);
          tx.commit();
        } catch (txErr) { tx.rollback(); throw txErr; }
        console.log(`[Hutang] Potongan tiket ${noTiket} dikembalikan (${daftar.length} pihak)`);
      } catch (e) {
        console.error('[Hutang] Gagal mengembalikan potongan tiket:', e.message);
      }
    }
    ...
    if (actionType === 'delete') {
      ...
      if (fullTrx) {
        ...
        const potongan = await kembalikanPotonganHutang(id, trx.no_tiket);
        const tx = beginTransaction();
        try {
          tulisPembalikHutang(tx, potongan);
          // Delete kas record
      ...
      // Potongan hutang dari timbangan-2 sebelumnya harus dikembalikan; angka
      // potongannya akan dihitung ulang saat tiket ini ditimbang keluar lagi.
      await balikkanHutangTiket(id, trx.no_tiket);
      await query(
      ...
      return jsonResponse(res, true, 'Transaksi dikembalikan ke status Timbangan Masuk');
    } else {
      if (trx.status === 'dibatalkan') return jsonResponse(res, false, 'Transaksi sudah dibatalkan');
      await refundKas(id, trx.no_tiket);
      // Tiket batal → hutang yang sempat dipotong otomatis dikembalikan
      await balikkanHutangTiket(id, trx.no_tiket);
      await query(
        `UPDATE transaksi_timbangan SET status='dibatalkan', status_bayar='belum_bayar', metode_bayar=NULL, tanggal_bayar=NULL, id_pembayaran=NULL,
         cancelled_at=NOW(), cancelled_by=?, cancel_reason=?, updated_at=NOW() WHERE id=?`,
        [user.id, reason, id]
      );
