...
/**
 * Catat mutasi kas yang menyertai mutasi hutang TUNAI, di dalam transaksi yang sama.
 *
 * Hanya dipakai bila uang benar-benar berpindah:
 *   - jenis 'bayar'  → orang menyetor pelunasan  → kas MASUK
 *   - jenis 'tambah' → kasbon diserahkan tunai   → kas KELUAR
 *
 * Mutasi non-tunai (hutang pupuk/barang, saldo awal pendaftaran) tidak lewat sini
 * supaya saldo kas tidak ikut bergerak untuk sesuatu yang bukan uang.
 *
 * Tautan dibuat dua arah (hutang_ledger.id_kas ↔ kas.id_hutang_ledger) agar baris
 * kasnya bisa dikunci dari penghapusan lepas di Buku Kas.
 */
function catatKasHutangTx(tx, p) {
  const [ledgerRow] = tx.execute(`SELECT last_insert_rowid() AS id`);
  const idLedger = ledgerRow[0]?.id;

  const [lastKas] = tx.execute(`SELECT saldo_setelah FROM kas ORDER BY id DESC LIMIT 1`);
  const saldoSebelum = lastKas.length ? parseFloat(lastKas[0].saldo_setelah) : 0;

  const isMasuk = p.jenis === 'bayar';
  const saldoSesudah = isMasuk ? saldoSebelum + p.jumlah : saldoSebelum - p.jumlah;

  // SOP keterangan: KATEGORI - NAMA  (lihat helpers/kasHelper.js)
  const kategori = `${isMasuk ? 'BAYAR HUTANG' : 'KASBON'} ${p.label}`;
  const keterangan = buildKeterangan(kategori, p.nama);

  const tglSql = p.tanggal ? '?' : `date('now','localtime')`;
  const tglParams = p.tanggal ? [p.tanggal] : [];

  const [insKas] = tx.execute(
    `INSERT INTO kas (tanggal, jenis, jumlah, keterangan, id_hutang_ledger, saldo_setelah, operator_id, created_at)
     VALUES (${tglSql}, ?, ?, ?, ?, ?, ?, datetime('now','localtime'))`,
    [...tglParams, isMasuk ? 'masuk' : 'keluar', p.jumlah, keterangan, idLedger,
     saldoSesudah, p.operatorId != null ? p.operatorId : null]
  );

  tx.execute(`UPDATE hutang_ledger SET id_kas = ? WHERE id = ?`, [insKas.insertId, idLedger]);
  return { idLedger, idKas: insKas.insertId, saldoKas: saldoSesudah };
}
...
async function mutasiHutang(req, res, jenis, defaultKet) {
  try {
    ...
    // is_tunai menentukan apakah uang benar-benar berpindah. Bila tidak dikirim,
    // dianggap tunai — perilaku yang paling lazim untuk kasbon dan setoran.
    const isTunai = req.body.is_tunai === undefined ? true
      : (req.body.is_tunai === true || req.body.is_tunai === 'true' || req.body.is_tunai === 1 || req.body.is_tunai === '1');

    // Nama pihak untuk keterangan di Buku Kas
    const pihak = await queryOne(`SELECT ${c.nameCol} AS nama FROM ${c.table} WHERE id = ?`, [id]);
    if (!pihak) return jsonResponse(res, false, `${c.label} tidak ditemukan`);

    let saldo, kasInfo = null;
    const tx = beginTransaction();
    try {
      saldo = catatHutangTx(tx, {
        type: req.params.type, partyId: id, jenis, jumlah, keterangan,
        sumber: 'manual', operatorId: req.session.user_id, tanggal
      });
      if (isTunai) {
        kasInfo = catatKasHutangTx(tx, {
          jenis, jumlah, label: c.label, nama: pihak.nama,
          operatorId: req.session.user_id, tanggal
        });
      }
      tx.commit();
    } catch (txErr) { tx.rollback(); throw txErr; }

    // Saldo berjalan dihitung ulang supaya kolom saldo di Buku Kas tetap benar
    // walaupun mutasinya dicatat mundur (backdate).
    if (kasInfo) await recalculateKasBalances();

    const kata = jenis === 'bayar' ? 'Pembayaran' : 'Penambahan';
    const efekKas = isTunai
      ? (jenis === 'bayar' ? ' Kas bertambah.' : ' Kas berkurang.')
      : ' Tidak menyentuh kas (non-tunai).';
    return jsonResponse(res, true, `${kata} hutang ${c.label} berhasil dicatat.${efekKas}`,
      { saldo, is_tunai: isTunai, saldo_kas: kasInfo ? kasInfo.saldoKas : null });
  } catch (err) { return jsonResponse(res, false, err.message); }
}
