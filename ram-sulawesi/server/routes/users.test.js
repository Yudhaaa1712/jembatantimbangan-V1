...
describe('users.js - User Management Routes Integration Tests', () => {
  ...
  test('DELETE /users/:id - should fail if trying to delete self', async () => {
    mockUser = { id: 1, username: 'admin', role: 'admin' };
  ...
  test('DELETE /users/:id - barisnya benar-benar dihapus, bukan dinonaktifkan', async () => {
    mockUser = { id: 1, username: 'admin', role: 'admin' };
    database.db.exec(`
      INSERT INTO users (id, username, nama_lengkap, password, role, status) VALUES
      (2, 'operator1', 'Old Name', 'hash2', 'operator', 'active');
    ...
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
    expect(res.body.message).toContain('dihapus permanen');

    // Verify DB — tidak boleh menyisakan baris berstatus 'inactive'
    const row = database.db.prepare(`SELECT * FROM users WHERE id = 2`).get();
    expect(row).toBeUndefined();
  });

  test('DELETE /users/:id - akun admin utama tidak bisa dihapus', async () => {
    mockUser = { id: 9, username: 'admin2', role: 'admin' };
    database.db.exec(`
      INSERT INTO users (id, username, nama_lengkap, password, role, status) VALUES
      (1, 'admin', 'Admin Utama', 'hash1', 'admin', 'active'),
      (9, 'admin2', 'Admin Kedua', 'hash9', 'admin', 'active');
    `);

    const res = await request(app).delete('/users/1');

    expect(res.body.success).toBe(false);
    expect(res.body.message).toContain('admin utama');
    expect(database.db.prepare(`SELECT * FROM users WHERE id = 1`).get()).toBeDefined();
  });
