...
const { buildKeterangan } = require('../helpers/kasHelper');
...
router.post('/ajax', async (req, res) => {
  ...
  try {
    switch (action) {
      ...
      case 'save_timbangan1': {
        ...
        if (req.body.is_langsir == 1 || req.body.is_langsir == '1' || req.body.is_langsir === true) {
          ...
          if (txRow) {
            ...
            if (detailCount.c == 0) {
              // Trip 1 dibuka dengan status 'bruto' — belum boleh dihargai sampai
              // taranya ditimbang saat mobil keluar.
              await query(
                `INSERT INTO transaksi_timbangan_langsir
                   (id_transaksi, urutan, id_kendaraan, no_polisi, nama_supir, berat_bruto, waktu_timbang, status, operator_id)
                 VALUES (?, 1, ?, ?, ?, ?, NOW(), 'bruto', ?)`,
                [txRow.id, idKendaraan, kendaraan.no_polisi, namaSuppir, beratT1, user.id]
              );
      ...
      // ── Append Langsir Trip (timbang BRUTO trip berikutnya) ──────────────
      case 'append_langsir_trip': {
        ...
        // Trip sebelumnya harus sudah ditimbang taranya. Kalau tidak, bruto akan
        // menumpuk tanpa pasangan tara dan netto ikut membengkak seperti dulu.
        const menggantung = await queryOne(
          `SELECT urutan, no_polisi FROM transaksi_timbangan_langsir
           WHERE id_transaksi = ? AND COALESCE(status,'bruto') <> 'selesai'
           ORDER BY urutan ASC LIMIT 1`, [idTransaksi]
        );
        if (menggantung) {
          return jsonResponse(res, false,
            `Trip ke-${menggantung.urutan} (${menggantung.no_polisi || '-'}) belum ditimbang tara. Timbang tara mobil itu dulu di menu Timbangan Keluar sebelum menambah trip baru.`);
        }

        // Plat & supir dicatat per trip karena mobil bisa berganti antar trip.
        const idKendaraanTrip = parseInt(req.body.id_kendaraan) || null;
        let platTrip = cleanInput(req.body.no_polisi || '').toUpperCase();
        let supirTrip = cleanInput(req.body.nama_supir || '');
        if (idKendaraanTrip && !platTrip) {
          const k = await queryOne(`SELECT no_polisi, nama_supir FROM kendaraan WHERE id = ?`, [idKendaraanTrip]);
          if (k) { platTrip = k.no_polisi; supirTrip = supirTrip || k.nama_supir; }
        }

        const newTripCount = (existing.jumlah_trip_langsir || 1) + 1;
        const accumulatedBruto = parseFloat(existing.berat_timbangan1 || 0) + newBruto;

        await query(
          `INSERT INTO transaksi_timbangan_langsir
             (id_transaksi, urutan, id_kendaraan, no_polisi, nama_supir, berat_bruto, waktu_timbang, status, operator_id)
           VALUES (?, ?, ?, ?, ?, ?, NOW(), 'bruto', ?)`,
          [idTransaksi, newTripCount, idKendaraanTrip, platTrip || null, supirTrip || null, newBruto, req.session.user_id]
        );
        await query(
          `UPDATE transaksi_timbangan SET berat_timbangan1 = ?, jumlah_trip_langsir = ?, updated_at = NOW() WHERE id = ?`,
          [accumulatedBruto, newTripCount, idTransaksi]
        );

        return jsonResponse(res, true,
          `Bruto trip ke-${newTripCount} tersimpan. Timbang taranya saat mobil keluar.`,
          { urutan: newTripCount });
      }

      // ── Daftar trip langsir sebuah tiket ─────────────────────────────────
      case 'get_langsir_trips': {
        const idTransaksi = parseInt(req.body.id_transaksi);
        if (!idTransaksi) return jsonResponse(res, false, 'ID transaksi tidak valid');

        const trips = await query(
          `SELECT id, urutan, no_polisi, nama_supir, berat_bruto, berat_tara, berat_netto,
                  waktu_timbang, waktu_tara, status
           FROM transaksi_timbangan_langsir WHERE id_transaksi = ? ORDER BY urutan ASC`,
          [idTransaksi]
        );
        const totalNetto = trips.reduce((a, t) => a + parseFloat(t.berat_netto || 0), 0);
        const belumTara = trips.filter(t => (t.status || 'bruto') !== 'selesai');

        return jsonResponse(res, true, 'Daftar trip langsir', {
          trips,
          total_bruto: trips.reduce((a, t) => a + parseFloat(t.berat_bruto || 0), 0),
          total_tara: trips.reduce((a, t) => a + parseFloat(t.berat_tara || 0), 0),
          total_netto: totalNetto,
          jumlah_trip: trips.length,
          trip_belum_tara: belumTara.length ? belumTara[0] : null
        });
      }

      // ── Timbang TARA satu trip langsir ───────────────────────────────────
      case 'save_langsir_tara': {
        const idTransaksi = parseInt(req.body.id_transaksi);
        const beratTara = parseFloat(req.body.berat_tara) || 0;
        if (!idTransaksi || beratTara <= 0) return jsonResponse(res, false, 'Berat tara tidak valid');

        const trip = await queryOne(
          `SELECT id, urutan, no_polisi, berat_bruto FROM transaksi_timbangan_langsir
           WHERE id_transaksi = ? AND COALESCE(status,'bruto') <> 'selesai'
           ORDER BY urutan ASC LIMIT 1`, [idTransaksi]
        );
        if (!trip) return jsonResponse(res, false, 'Tidak ada trip yang menunggu timbang tara');

        const bruto = parseFloat(trip.berat_bruto || 0);
        if (beratTara >= bruto) {
          return jsonResponse(res, false,
            `Tara (${beratTara.toLocaleString('id-ID')} kg) tidak boleh sama atau lebih besar dari bruto trip ke-${trip.urutan} (${bruto.toLocaleString('id-ID')} kg).`);
        }
        const nettoTrip = bruto - beratTara;

        await query(
          `UPDATE transaksi_timbangan_langsir
           SET berat_tara = ?, berat_netto = ?, waktu_tara = NOW(), status = 'selesai', operator_id = ?
           WHERE id = ?`,
          [beratTara, nettoTrip, req.session.user_id, trip.id]
        );

        // Akumulasi tara disimpan di tiket induk supaya kolom berat_tara tiket
        // mencerminkan seluruh tara yang benar-benar dipotong.
        const agg = await queryOne(
          `SELECT COALESCE(SUM(berat_tara),0) AS tara, COALESCE(SUM(berat_netto),0) AS netto,
                  SUM(CASE WHEN COALESCE(status,'bruto') <> 'selesai' THEN 1 ELSE 0 END) AS sisa
           FROM transaksi_timbangan_langsir WHERE id_transaksi = ?`, [idTransaksi]
        );
        await query(
          `UPDATE transaksi_timbangan SET berat_tara = ?, updated_at = NOW() WHERE id = ?`,
          [agg.tara, idTransaksi]
        );

        return jsonResponse(res, true,
          `Tara trip ke-${trip.urutan} tersimpan. Netto trip ini ${nettoTrip.toLocaleString('id-ID')} kg.`,
          { urutan: trip.urutan, netto_trip: nettoTrip, total_netto: agg.netto, trip_belum_tara: agg.sisa }
        );
      }
      ...
      case 'save_timbangan2': {
        ...
        const isLangsir = transaksi.is_langsir === 1;

        let netto, updateBruto, updateTara, beratT2Simpan = beratT2;

        if (isLangsir) {
          // Tiket langsir: netto adalah JUMLAH netto tiap trip, masing-masing sudah
          // dikurangi taranya sendiri. Tidak boleh dihitung dari bruto akumulasi
          // dikurangi satu tara — tara trip ke-2 dan seterusnya akan hilang dan
          // ikut terhitung sebagai buah.
          const agg = await queryOne(
            `SELECT COUNT(*) AS jumlah,
                    SUM(CASE WHEN COALESCE(status,'bruto') <> 'selesai' THEN 1 ELSE 0 END) AS belum_tara,
                    COALESCE(SUM(berat_bruto),0) AS bruto,
                    COALESCE(SUM(berat_tara),0)  AS tara,
                    COALESCE(SUM(berat_netto),0) AS netto
             FROM transaksi_timbangan_langsir WHERE id_transaksi = ?`, [idTransaksi]
          );
          if (!agg || agg.jumlah === 0) {
            return jsonResponse(res, false, 'Data trip langsir tidak ditemukan.');
          }
          if (agg.belum_tara > 0) {
            return jsonResponse(res, false,
              `Masih ada ${agg.belum_tara} trip yang belum ditimbang tara. Selesaikan tara semua trip sebelum menyimpan tiket.`);
          }
          netto = parseFloat(agg.netto);
          updateBruto = parseFloat(agg.bruto);
          updateTara = parseFloat(agg.tara);
          // berat_timbangan2 pada tiket langsir tidak bermakna tunggal — yang
          // berlaku adalah tara tiap trip. Disimpan sebagai total tara.
          beratT2Simpan = updateTara;
        } else {
          netto = isJual
            ? (beratT2 - transaksi.berat_timbangan1)
            : (transaksi.berat_timbangan1 - beratT2);

          // For 'beli', the true Bruto is T1. Tara is T2 (single).
          updateBruto = isJual ? beratT2 : transaksi.berat_timbangan1;
          updateTara  = isJual ? transaksi.berat_timbangan1 : beratT2;
        }
        ...
        // ── Nilai bersih tiket & status pembayaran ───────────────────────────
        // total_akhir = yang benar-benar harus dibayarkan ke supplier setelah
        // semua potongan. Disimpan supaya halaman Pembayaran tidak perlu
        // menghitung ulang (dan tidak bisa melenceng dari nota yang dicetak).
        const potJln = parseFloat(transaksi.potongan_jalan || 0);
        const potPpk = parseFloat(transaksi.potongan_pupuk_rp || 0);
        const potHutSupir = isHutangActive ? finalPotonganHutang : parseFloat(transaksi.potongan_hutang_rp || 0);
        const totalAkhirTiket = Math.max(0, totalHarga - (potJln + potPpk + potHutSupir + finalPotonganHutangSupplier + potonganMuat));

        // metode_bayar diisi 'tunai'/'transfer' bila uang diserahkan saat itu juga.
        // Kosong = supplier belum dibayar → tiket masuk antrian Pembayaran dan kas
        // TIDAK bergerak. Mode 'jual' selalu tunai karena uang justru masuk.
        const metodeInput = cleanInput(req.body.metode_bayar || '').toLowerCase();
        const bayarSekarang = isJual || metodeInput === 'tunai' || metodeInput === 'transfer';
        const metodeBayarTiket = bayarSekarang ? (metodeInput === 'transfer' ? 'transfer' : 'tunai') : null;
        const statusBayarTiket = bayarSekarang ? 'lunas' : 'belum_bayar';
        ...
        try {
          tx.execute(
            `UPDATE transaksi_timbangan SET
               ...
               total_akhir = ?, status_bayar = ?, metode_bayar = ?,
               tanggal_bayar = CASE WHEN ? = 'lunas' THEN date('now','localtime') ELSE NULL END,
             ...
             WHERE id = ?`,
            [idCustomer, beratT2Simpan, updateBruto, updateTara,
             ...
             totalAkhirTiket, statusBayarTiket, metodeBayarTiket, statusBayarTiket,
        ...
        try {
          const totalAkhir = totalAkhirTiket;

          // Kas hanya bergerak bila uang benar-benar berpindah saat ini juga.
          // Kalau supplier belum dibayar (bayar nanti / dirapel akhir hari), tiket
          // cuma jadi kewajiban — pencatatan kasnya menyusul lewat /pembayaran.
          //
          // PENCATATAN BRUTO
          // Potongan hutang dicatat terpisah sebagai uang MASUK, bukan sekadar
          // mengecilkan uang keluar. Contoh tiket Rp 10jt dengan potong hutang
          // supir Rp 3jt (dibayar tunai):
          //     KELUAR  Rp 10.000.000  PEMBELIAN TBS - UJANG - TKT... (TUNAI)
          //     MASUK   Rp  3.000.000  POTONG HUTANG SUPIR - UJANG - TKT...
          // Saldo tetap berkurang Rp 7jt, tapi pelunasan hutangnya terbaca
          // sebagai pemasukan di Laporan Keuangan.
          //
          // Hanya potongan yang benar-benar pelunasan hutang (tercatat di
          // hutang_ledger) yang dibruto-kan. Potongan jalan/pupuk/muat tetap
          // mengurangi nilai tiket karena bukan uang yang kita terima kembali.
          const potonganHutangKas = isHutangActive ? finalPotonganHutang : 0;
          const totalBruto = totalAkhir + potonganHutangKas + finalPotonganHutangSupplier;

          if (totalBruto > 0 && bayarSekarang) {
            ...
            const noTiket = tiketInfo?.no_tiket || null;

            // SOP keterangan: KATEGORI - NAMA - REFERENSI (lihat helpers/kasHelper.js)
            const materialLabel = (tiketInfo?.jenis_material || 'BUAH');

            // Daftar baris kas yang akan ditulis untuk tiket ini
            const barisKas = [{
              jenis: isBeli ? 'keluar' : 'masuk',
              jumlah: totalBruto,
              keterangan: buildKeterangan(
                `${isBeli ? 'PEMBELIAN' : 'PENJUALAN'} ${materialLabel}`,
                tiketInfo?.nama_supir, noTiket, isBeli ? metodeBayarTiket : null)
            }];
            if (potonganHutangKas > 0) {
              barisKas.push({
                jenis: 'masuk', jumlah: potonganHutangKas,
                keterangan: buildKeterangan('POTONG HUTANG SUPIR', tiketInfo?.nama_supir, noTiket)
              });
            }
            if (finalPotonganHutangSupplier > 0) {
              barisKas.push({
                jenis: 'masuk', jumlah: finalPotonganHutangSupplier,
                keterangan: buildKeterangan('POTONG HUTANG SUPPLIER', tiketInfo?.nama_supplier, noTiket)
              });
            }

            // Saldo berjalan dihitung berurutan supaya kolom saldo di Buku Kas benar
            const lastKas = await queryOne(`SELECT saldo_setelah FROM kas ORDER BY id DESC LIMIT 1`);
            let saldoBerjalan = lastKas ? parseFloat(lastKas.saldo_setelah) : 0;
            ...
            for (const b of barisKas) {
              ...
              await query(
                `INSERT INTO kas (tanggal, jenis, jumlah, keterangan, id_transaksi, no_tiket, saldo_setelah, operator_id, created_at)
                 VALUES (CURDATE(), ?, ?, ?, ?, ?, ?, ?, NOW())`,
                [b.jenis, b.jumlah, b.keterangan, idTransaksi, noTiket, saldoBerjalan, req.session.user_id]
              );
            }

            const saldoSesudah = saldoBerjalan;
            console.log(`[Kas] Auto-${isBeli ? 'deduct' : 'deposit'}: ${isBeli ? '-' : '+'}Rp ${totalBruto.toLocaleString('id-ID')} bruto (${noTiket}), ${barisKas.length} baris. Sisa: Rp ${saldoSesudah.toLocaleString('id-ID')}`);

            // Sync ke Google Sheet — setiap baris kas dikirim terpisah supaya
            // isi sheet sama persis dengan Buku Kas di aplikasi.
            ...
            if (settingKas && settingKas.setting_value && settingKas.setting_value.startsWith('http')) {
              let saldoSheet = saldoSesudah - barisKas.reduce(
                (s, b) => s + (b.jenis === 'masuk' ? 1 : -1) * b.jumlah, 0);
              for (const b of barisKas) {
                saldoSheet += (b.jenis === 'masuk' ? 1 : -1) * b.jumlah;
                try {
                  ...
                  const sheetDataKas = {
                    ...
                    debit: b.jenis === 'masuk' ? rupiah(b.jumlah) : '',
                    kredit: b.jenis === 'keluar' ? rupiah(b.jumlah) : '',
                    saldo: rupiah(saldoSheet),
        ...
        return jsonResponse(
          ...
          true,
          bayarSekarang
            ? 'Data Timbangan Keluar berhasil disimpan (LUNAS)'
            : 'Data Timbangan Keluar disimpan. Status BELUM LUNAS — bayar lewat menu Pembayaran.',
          {
            id_transaksi: idTransaksi,
            no_tiket: transaksi.no_tiket,
            total_akhir: totalAkhirTiket,
            status_bayar: statusBayarTiket,
            metode_bayar: metodeBayarTiket
          }
        );
      }
...
router.post('/save-timbangan1', async (req, res) => {
  ...
  try {
    ...
    if (isLangsir === 1) {
      const txRow = await queryOne(`SELECT id FROM transaksi_timbangan WHERE no_tiket = ?`, [noTiket]);
      if (txRow) {
        // Trip 1 dibuka dengan status 'bruto'; taranya ditimbang saat mobil keluar.
        await query(
          `INSERT INTO transaksi_timbangan_langsir
             (id_transaksi, urutan, no_polisi, nama_supir, berat_bruto, waktu_timbang, status, operator_id)
           VALUES (?, 1, ?, ?, ?, NOW(), 'bruto', ?)`,
          [txRow.id, noKendaraan, namaPengemudi, berat, user.id]
        );
      }
    }
