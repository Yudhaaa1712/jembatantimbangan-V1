...
describe('pembayaran.js - Pembayaran supplier (rapel beberapa tiket)', () => {
  ...
  test('POST /hapus - baris pembayaran benar-benar hilang, saldo utuh, tiket belum lunas', async () => {
    const ids = [
    ...
    const hapus = await request(app)
      .post('/pembayaran-api/hapus')
      .send({ id: simpan.body.data.id });

    expect(hapus.body.success).toBe(true);
    expect(saldoKas()).toBe(50000000);
    expect(database.db.prepare(`SELECT COUNT(*) c FROM kas WHERE jenis='keluar'`).get().c).toBe(0);
    ...
    // Hapus artinya hapus: header maupun rinciannya tidak boleh tersisa
    expect(database.db.prepare(`SELECT COUNT(*) c FROM pembayaran`).get().c).toBe(0);
    expect(database.db.prepare(`SELECT COUNT(*) c FROM pembayaran_detail`).get().c).toBe(0);
  });

  test('POST /hapus - pembayaran yang sudah dihapus tidak bisa dihapus lagi', async () => {
    const id = buatTiket('TKT-001', 'BM 1', idSupplier, 1000000);
    const simpan = await request(app)
      .post('/pembayaran-api/simpan')
      .send({ metode: 'tunai', tiket_ids: [id] });
    await request(app).post('/pembayaran-api/hapus').send({ id: simpan.body.data.id });

    const lagi = await request(app).post('/pembayaran-api/hapus').send({ id: simpan.body.data.id });

    expect(lagi.body.success).toBe(false);
    expect(lagi.body.message).toMatch(/tidak ditemukan/i);
    expect(saldoKas()).toBe(50000000);
  });

  test('POST /batal - alias lama tetap jalan dan ikut menghapus permanen', async () => {
    const id = buatTiket('TKT-001', 'BM 1', idSupplier, 1000000);
    const simpan = await request(app)
      .post('/pembayaran-api/simpan')
      .send({ metode: 'tunai', tiket_ids: [id] });
