  ...
  test('POST /timbangan/ajax (save_timbangan2) with auto-deduct kas', async () => {
    ...
    const res = await request(app)
      ...
      .send({
        ...
        harga_per_kg: 2000,
        keterangan: 'Bersih',
        // Sejak ada opsi "bayar nanti", kas hanya bergerak bila metode bayarnya
        // memang tunai/transfer. Tanpa field ini tiket dianggap belum dibayar
        // dan tidak boleh ada baris kas sama sekali.
        metode_bayar: 'tunai'
      });
    ...
    // SOP keterangan Buku Kas: KATEGORI - NAMA - REFERENSI (helpers/kasHelper.js)
    expect(kas.keterangan).toBe('PEMBELIAN TBS - JONO - TKT-260616-002 (TUNAI)');
    expect(kas.saldo_setelah).toBe(-8350000);
  });

  test('POST /timbangan/ajax (save_timbangan2) tanpa metode bayar - tiket jadi hutang, kas tidak bergerak', async () => {
    database.db.exec(`
      INSERT INTO supplier (id, kode_supplier, nama_supplier) VALUES (3, 'SUP3', 'Petani C');
      INSERT INTO transaksi_timbangan
      (id, no_tiket, status, berat_timbangan1, no_polisi, nama_supir, id_supplier, jenis_material, harga_per_kg)
      VALUES (13, 'TKT-260616-003', 'timbang_1', 10000, 'BK 2222 EE', 'Sirun', 3, 'TBS', 2000);
    `);
    ...
    const trx = database.db.prepare(`SELECT * FROM transaksi_timbangan WHERE id = 13`).get();
    expect(trx.status).toBe('selesai');
    expect(trx.status_bayar).toBe('belum_bayar');

    // Belum dibayar → belum ada uang keluar. Pencatatan kasnya menyusul
    // lewat menu Pembayaran.
    const kas = database.db.prepare(`SELECT * FROM kas WHERE id_transaksi = 13`).get();
    expect(kas).toBeUndefined();
  });
});
