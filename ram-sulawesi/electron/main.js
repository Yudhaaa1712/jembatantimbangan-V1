...
function createWindow() {
  ...
  mainWindow.once('ready-to-show', () => {
    // Buka langsung memenuhi layar. maximize() dipakai (bukan setFullScreen)
    // supaya menu Aplikasi/Tampilan/Bantuan tetap terlihat; layar penuh
    // sepenuhnya tetap tersedia lewat F11.
    mainWindow.maximize();
    mainWindow.show();
  ...
  // Semua halaman cetak milik aplikasi dibuka sebagai jendela Electron (modal),
  // BUKAN dilempar ke browser. Hanya tautan di luar daftar ini yang dianggap
  // eksternal. Tambahkan path cetak baru ke sini, kalau tidak akan lari ke browser.
  const PRINT_PATHS = [
    '/print-ticket',
    '/print-pengiriman',
    '/surat-jalan',
  ...
  mainWindow.webContents.setWindowOpenHandler(({ url }) => {
    ...
    try {
      // Cocokkan pada pathname saja supaya query string tidak bisa mengelabui.
      const u = new URL(url, `http://localhost:${SERVER_PORT}`);
      isInternal = u.hostname === 'localhost' && PRINT_PATHS.some(p => u.pathname.startsWith(p));
    } catch (e) {
      isInternal = PRINT_PATHS.some(p => String(url).includes(p));
    }
    ...
    if (isInternal) {
      return {
        ...
        overrideBrowserWindowOptions: {
          ...
          parent: mainWindow,
          modal: true,
          // Preload ikut dipasang supaya halaman struk bisa memanggil
          // printStruk() — mencetak tanpa margin bawaan dialog.
          webPreferences: {
            preload: path.join(__dirname, 'preload.js'),
            nodeIntegration: false,
            contextIsolation: true
...
// Cetak jendela struk yang memanggil, bukan jendela utama.
//
// window.print() biasa memakai margin bawaan dialog Chromium (± 1 cm di setiap
// sisi) yang menimpa "@page { margin: 0 }", sehingga isi struk turun dari tepi
// atas kertas. marginType 'none' mematikannya, jadi posisi cetak sepenuhnya
// ditentukan Geser X / Geser Y di Pengaturan.
ipcMain.handle('print-struk', async (event) => {
  return new Promise((resolve) => {
    event.sender.print(
      { silent: false, printBackground: true, margins: { marginType: 'none' } },
      (success, errorType) => {
        if (!success && errorType !== 'cancelled') console.error('Print struk gagal:', errorType);
        resolve(success);
      }
    );
  });
