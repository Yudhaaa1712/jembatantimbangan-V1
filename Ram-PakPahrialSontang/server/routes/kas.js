  ...
  try {
    ...
    const formattedRows = rows.map(row => {
      ...
      if (row.id_transaksi && row.jenis_material) {
        ...
        const supplier = (row.nama_supplier || '').toUpperCase();
        keterangan = `${material} ${supplier}`.trim();
      }
