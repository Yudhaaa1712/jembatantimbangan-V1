...
router.post('/clear-data', requireRole('admin'), async (req, res) => {
  try {
    ...
    try {
      ...
      tx.execute(`UPDATE supir SET total_hutang = 0`);
      ...
      tx.commit();
      return jsonResponse(res, true, 'Semua data transaksi, kas, supplier, material kustom, dan pengguna non-admin berhasil dikosongkan.');
    } catch (txErr) {
      tx.rollback();
      throw txErr;
    }
